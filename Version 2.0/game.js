...
function doRebirth() {
  ...
  coinTotal = Math.max(0, coinTotal - cost);
  rebirthCount = next;
...
function startRolling() {
  ...
  const flasher = startFlashingIcon(resultEl, 50);
  ...
  setTimeout(() => {
    ...
    if (flasher && typeof flasher.stop === 'function') flasher.stop(character);
    ...
    if (upgrades.secondRoll) {
      // perform a second roll simultaneously (no golden multiplier applied)
      const character2 = weightedRandom(allCharacters, { goldenMultiplier: 1 });
